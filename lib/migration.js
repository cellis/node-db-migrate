...
Migration.prototype.sqlFileLoaderTemplate = function () {
  return [
    ...
    "var path = require('path');",
    'var Promise;',
    'var replaceStrings = process.env.REPLACE_MIGRATION_STRINGS;',
    '',
    '/**',
    '  * We receive the dbmigrate dependency from dbmigrate initially.',
    '  * This enables us to not have to rely on NODE_PATH.',
    ...
    '     ',
    '      if(replaceStrings) {',
    "        var replaceStringsArr = replaceStrings.split('|');",
    '        replaceStringsArr.forEach(function(replaceStr) {',
    "           var replacePair = replaceStr.split(',');",
    "           data = data.replace(new RegExp(replacePair[0], 'g'), replacePair[1]);",
    '        });',
    '      }',
    '      data = data.replace(/__(.*?)__/g, function(group1, group2) {',
    '        return process.env[group2] || group2;',
    '      })',
...
Migration.prototype.sqlFileLoaderIgnoreOnInitTemplate = function () {
  return [
    ...
    "  var filePath = path.join(__dirname + '/sqls/" +
      ...
      "-down.sql');",
    '  return new Promise( function( resolve, reject ) {',
    "    fs.readFile(filePath, {encoding: 'utf-'}, function(err,data){",
    '      if (err) return reject(err);',
    "      console.log('received data: ' + data);",
    '',
    '      resolve(data);',
